<template>
  <section>
    <div :style="`background: ${options.color}`" class="filterPanel">
      <h2>Сортировка</h2>
      <div class="tooltipsSort">
        <v-tooltip text="Сортировка по возрастанию">
          <template v-slot:activator="{ props }">
            <v-btn v-bind="props" icon="mdi-sort-descending" density="compact" variant="tonal" class="sort-btn"
              color="white" @click="sortListDown" />
          </template>
        </v-tooltip>
        <v-tooltip text="Сортировка по убыванию">
          <template v-slot:activator="{ props }">
            <v-btn v-bind="props" icon="mdi-sort-ascending" density="compact" variant="tonal" class="sort-btn"
              color="white" @click="sortListUp" />
          </template>
        </v-tooltip>
        <v-tooltip text="Без сортировки">
          <template v-slot:activator="{ props }">
            <v-btn v-bind="props" icon="mdi-sort-variant-remove" density="compact" variant="tonal" class="sort-btn"
              color="white" @click="sortListDefault" />
          </template>
        </v-tooltip>
      </div>
    </div>
